...
# Cumulative per-model match scores (reset on app restart)
...
PREDICTION_VOTING_WINDOW = 3  # Last N predictions for majority voting
...
def update_values(t, h, p, uv, pres):
    ...
    if model and label_encoder and scaler:
        try:
            # Features must match scaler.feature_names_in_:
            # Temperature, Humidity, Precipitation (%), UV Index, Pressure, hour_of_day, day_of_week, month
            ...
            print(f"[FEAT] Current → T={smoothed_values['temperature']:.2f} H={smoothed_values['humidity']:.2f} "
            ...
            # Get prediction and apply majority voting
            ...
            sensor_data["confidence"] = round(confidence, 1)
            print(f"✅ Current Weather: {stabilized_prediction} ({confidence:.1f}%)")
        except Exception as e:
    ...
    # 2-Hour Forecast Prediction (5 sensors + 3 time features)
    if forecast_model and forecast_encoder and forecast_scaler:
        try:
            ...
            print(f"[FEAT] Forecast → T={smoothed_values['temperature']:.2f} H={smoothed_values['humidity']:.2f} "
                  f"P={smoothed_values['precipitation']:.2f} UV={smoothed_values['uv_index']:.2f} "
                  f"Pres={smoothed_values['pressure']:.2f} hr={hour_of_day} dow={day_of_week} mo={month}")
            ...
            print(f"✅ 2-Hour Forecast: {stabilized_forecast}")
        except Exception as e:
            print(f"⚠️  Forecast error: {type(e).__name__} - {e}")
            sensor_data["forecast_2hr"] = "N/A"
    else:
        sensor_data["forecast_2hr"] = "Model not loaded"
    
    # ── Multi-model comparison predictions ──────────────────────────────────
    # All models share the same scaler and encoders (same 8 features).
    if scaler and label_encoder and forecast_encoder:
        try:
            _c_input = pd.DataFrame([[
                smoothed_values["temperature"],
                smoothed_values["humidity"],
                smoothed_values["precipitation"],
                smoothed_values["uv_index"],
    ...
    # ── Score each model against live WeatherAPI ground truth ───────────────
    ...
    if actual:
        for _key in ("decision_tree", "gradient_boosting", "knn", "random_forest"):
            ...
            if _pred and _pred == actual:
                sensor_data["model_scores"][_key] = sensor_data["model_scores"].get(_key, 0) + 1
        _best_key = max(sensor_data["model_scores"], key=sensor_data["model_scores"].get)
...
_SIM_CONDITIONS = {
    ...
    'Sunny':  (28, 35,  30, 45,  0,  5,  8, 12, 1010, 1020),
    'Cloudy': (18, 25,  60, 75, 10, 30,  2,  6, 1000, 1012),
    'Rainy':  (12, 20,  80, 95, 50, 90,  0,  3,  990, 1005),
}
