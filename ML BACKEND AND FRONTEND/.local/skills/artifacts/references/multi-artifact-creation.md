...
The design subagent is effective for react-vite frontends, mockup sandboxes, and media generation (images, videos). However, it cannot produce good results for:
...
Check each artifact's skill for whether to delegate to the design subagent.
...
1. Create react-vite artifact first (its skill uses an async design subagent, creating an idle window) → OpenAPI (covering BOTH web + mobile) → codegen → launch async design subagent
2. While the design subagent runs: build all shared backend routes for both artifacts
3. Backend done, design subagent still running → create expo artifact → build it completely (expo is built synchronously by the main loop)
4. Design subagent finishes → fix issues → restart all workflows in parallel → check logs → present both
